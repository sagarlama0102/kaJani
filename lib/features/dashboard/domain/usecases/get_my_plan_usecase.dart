...
final getMyPlansUsecaseProvider = Provider<GetMyPlansUsecase>((ref) {
  return GetMyPlansUsecase(repository: ref.read(planRepositoryProvider));
});

class GetMyPlansUsecase implements UsecaseWithoutParams<List<PlanEntity>> {
  final IPlanRepository _repository;

  GetMyPlansUsecase({required IPlanRepository repository})
      : _repository = repository;

  @override
  Future<Either<Failure, List<PlanEntity>>> call() {
    return _repository.getMyPlans();
  }
}